...
For this to work, we have to read raw-values pretty often (because we average them, and "chase" them). Using "delay" will get in the way of this. Basically, don't use "delay" (see "wait_for" in here)!
  ...
  * 1 Sensor, Function Interface
  ...
  const int fast = 5; // experiment to find this, removes noise. constant: just a named value
  const int slow = 40; // experiment to find this, to get a good difference. constant: just a named value
  const int significant = 50; // experiment to find this, so false and true can be determined reliably.
  ...
  ** Details
    boolean auto_bilevel( 
      const int raw_value, # the noisy raw-value 
      const int fast, # the smoothing factor for the fast smoother.
      const int slow, # the smoothing factor for the slow smoother,
      const int significant, # the difference to trigger a change
...
// template (type T)
boolean auto_bilevel(const long raw, const int fast_factor, const int slow_factor, const long significant) {
  static long fast_smooth = raw;
  static long slow_smooth = 0;
...
long exponential_smooth(const long raw, const int factor) {
  static long smooth = raw;
  smooth = int(raw / factor + smooth - smooth / factor);
  return smooth;
  }

boolean auto_bilevel_graph(const long raw, const int fast_factor, const int slow_factor, const long significant) {
  // has to be floats because of rounding in integer world: will fail to "converge" etc.
  static float fast_smooth = raw;
  static float slow_smooth = raw;
  ...
  // exponential decay, i.e. "Smoothing" but it takes time. aka "chases values slowly"
  fast_smooth = (raw / (fast_factor+0.0) + fast_smooth - fast_smooth / fast_factor);
  slow_smooth = (raw / (slow_factor+0.0) + slow_smooth - slow_smooth / slow_factor);

  Serial.print(raw); Serial.print(" ");
  Serial.print(fast_smooth); Serial.print(" ");
  Serial.print(slow_smooth); Serial.print(" ");
  Serial.print(fast_smooth - significant); Serial.print(" ");
  ...
  if ( (fast_smooth - slow_smooth) >= significant ) {
    Serial.println(slow_smooth + 1);
    last = true;
  }
  else if ( (fast_smooth - slow_smooth) <= -significant ) {
    Serial.println(slow_smooth);
    last = false;
  }
  else {
    Serial.println(slow_smooth + (last ? 1 : 0));
    }
  return last;
} 
